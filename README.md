...
4. **GitHub Actions** запускает скрипт каждый день в 07:00 UTC.
...
По умолчанию workflow запускается в **12:40 UTC** (15:40 Europe/Minsk) каждый день. Чтобы изменить время, отредактируйте cron в `.github/workflows/daily-summary.yml`:

```yaml
- cron: '40 12 * * *'  # минуты часы (UTC)
```

Примеры:

- `0 6 * * *` — 06:00 UTC (09:00 Minsk)
- `40 12 * * *` — 12:40 UTC (15:40 Minsk)
- `0 21 * * *` — 21:00 UTC (00:00 Minsk следующего дня)
